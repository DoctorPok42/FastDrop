...
const sendFile = (
    ...
    if (files.length > 1) {
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                reader.onload = async () => {
                  ...
                  await socket.emit('fileUploadLarge', fileBuffer, file.name, targetUser, username, i, files.length);
                };
        ...
        try {
            ...
            reader.onload = () => {
                const fileBuffer = Buffer.from(reader.result as ArrayBuffer);
                socket.emit('fileUpload', fileBuffer, file.name, targetUser, username);
            };
